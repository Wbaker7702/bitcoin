        ...
        for (int bit = 0; bit < VERSIONBITS_NUM_BITS; bit++) {
            ...
            if (state == ThresholdState::ACTIVE || state == ThresholdState::LOCKED_IN) {
                ...
                if (state == ThresholdState::ACTIVE) {
                    ...
                    auto& notifications = static_cast<node::KernelNotifications&>(m_chainman.GetNotifications());
                    notifications.warningSet(kernel::Warning::UNKNOWN_NEW_RULES_ACTIVATED, warning);
                } else {
    ...
    std::multimap<uint256, FlatFilePos>* blocks_with_unknown_parent)
{
    ...
    try {
        ...
        bilingual_str error_msg = strprintf(_("System error while loading external block file: %s"), e.what());
...
bool ChainstateManager::ValidatedSnapshotCleanup()
{
    ...
    auto rename_failed_abort = [this](
        ...
        bilingual_str error_msg = strprintf(_(
            "Rename of '%s' -> '%s' failed. "
