...
bool BlockManager::LoadBlockIndex(const std::optional<uint256>& snapshot_blockhash)
{
    ...
    if (snapshot_blockhash) {
        ...
        if (!maybe_au_data) {
            bilingual_str error_msg = strprintf(_("Assumeutxo data not found for the given blockhash '%s'."), snapshot_blockhash->ToString());
            m_opts.notifications.fatalError(error_msg.original, error_msg);
            return false;
        }
...
bool BlockManager::FlushBlockFile(int blockfile_num, bool fFinalize, bool finalize_undo)
{
    ...
    if (!m_block_file_seq.Flush(block_pos_old, fFinalize)) {
        bilingual_str error_msg = _("Flushing block file to disk failed. This is likely the result of an I/O error.");
        m_opts.notifications.flushError(error_msg.original);
        success = false;
    }
...
FlatFilePos BlockManager::SaveBlockToDisk(const CBlock& block, int nHeight)
{
    ...
    if (!WriteBlockToDisk(block, blockPos)) {
        bilingual_str error_msg = _("Failed to write block.");
        m_opts.notifications.fatalError(error_msg.original, error_msg);
        return FlatFilePos();
    }
    return blockPos;
